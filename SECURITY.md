...
prompt templates, and MCP server configurations for AZ-104 certification study.
It does not contain application code, backend services, or deployable software.

The MCP server configurations in `.vscode/mcp.json` reference API keys
exclusively through VS Code input prompts (`${input:...}` variables). No secrets
are hardcoded in any tracked file. The `.gitignore` file excludes `.env` files,
private keys, and other sensitive material from version control.
...
- **References to real exam content** that compromise the integrity of the
  Microsoft AZ-104 certification exam. This repository must contain only
...
- Outdated Azure terminology or product name changes.
